...
def thread_fun(client)
	...
	while disconnect == false
		...
		if lines[0].include? "CHAT:"
			...
			puts lines
			puts client_details
			#@@myChatrooms[client_details[0].to_i].send_message(lines)
			puts "sending message to chatroom"
			@@myChatrooms[0].send_message("hai")
...
@@myChatrooms[0].add_user(Client.new("hai", 0, 0,"micky", 123, client))
@@myChatrooms[0].send_message("hai")
